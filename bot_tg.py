...
def start_message(message):
    user = crud.get_user_by_tg_id(message.from_user.id)
    if not user:
...
def wallet(message):
    ...
    markup.add(btn1)
    text = f'Ваш баланс: {(wallet["balance"]/100000000):.8f} BTC\n' \
...
def history(message):
    ...
    btn1 = telebot.types.KeyboardButton('Меню')
    markup.add(btn1)
    transactions = client.get_user_transactions_by_id(client.get_user_by_tg_id(message.from_user.id)['id'])
...
def total_balance(message):
    ...
    text = f'Общий баланс: {(balance/100000000):.8f} BTC'
    bot.send_message(message.chat.id, text, reply_markup=markup)
